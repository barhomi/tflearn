...
def make_balanced_batches(samples_size, batch_size, labels):
    ...
    from nqlearn.tools.utils import start_progressbar, end_progressbar, update_progressbar
    def chunkify(lst, n):
    ...
    pbar = start_progressbar(nb_batch, 'Balanced batches %i' % (nb_batch))
    l_all_ind = []
    for i_batch in range(nb_batch):
        perm_cl = np.random.permutation(l_classes)
        l_ind = []
        update_progressbar(pbar, i_batch)
